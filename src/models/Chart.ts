...
import type { Canvas, ChartConfig, Data, Layer, LayerType } from '@/types'
...
export default class Chart {
  ...
  public getStackedData(keys: string[]) {
    return stack().keys(keys).order(stackOrderNone)(this.data as any)
  }
  ...
  public getKeys(idx = -1, type: string | null = null) {
  ...
  public getStackedKeys(idx = -1, type: string | null = null) {
    const keys = this.layers
      .filter((l) => {
