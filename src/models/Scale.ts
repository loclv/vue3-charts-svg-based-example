import type { AxisConfig, Data, ScaleType } from '@/types'
...
export default class Scale {
  ...
  public updateDomain(
    data: Data[],
    stackedData: Series<any, string>[],
    keys: string[]
  ) {
    const domain =
      this.config.type === 'band'
        ? this.categoryDomain(data, keys)
        : this.numericDomain(data, stackedData, keys)
  ...
  private numericDomain(
    ...
    const values: number[] = keys.reduce(
      (arr, key) => arr.concat(getCol(key, data)),
      []
    )

    const valueMax: number =
      max(stackedData, (arr) => max(arr, (x) => max(x))) || 0
    const valueMin: number =
      min(stackedData, (arr) => min(arr, (x) => min(x))) || 0

    const [dataMin, dataMax] = extent(
      Array.from(new Set(values.concat([valueMin, valueMax])))
    )
    const yMin = eval(`
